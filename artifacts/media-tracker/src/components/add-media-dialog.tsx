...
export function AddMediaDialog({ open, onClose, defaultListType = "library" }: Props) {
  ...
  const [ratings, setRatings] = useState({ story: 0, art: 0, character: 0, worldBuilding: 0, uniqueness: 0, enjoyment: 0 });
  ...
  useEffect(() => {
    if (open) {
      ...
      setGenres([]);
      setCandidates(null);
      setRatings({ story: 0, art: 0, character: 0, worldBuilding: 0, uniqueness: 0, enjoyment: 0 });
      setSourceAccuracy(0);
      setHasSource(false);
    }
  ...
  const onSubmit = (values: FormValues) => {
    createMedia.mutate(
      ...
      rating: parseFloat(calcAverage()),
      ratingStory: ratings.story || null, ratingArt: ratings.art || null, ratingCharacter: ratings.character || null,
      ratingWorldBuilding: ratings.worldBuilding || null, ratingUniqueness: ratings.uniqueness || null, ratingEnjoyment: ratings.enjoyment || null,
      ratingSourceAccuracy: (watchedCategory === "anime" && hasSource) ? sourceAccuracy || null : null,
      } },
      {
        onSuccess: () => {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex items-center justify-between pb-2 border-b border-border">
              ...
              <div className="px-2.5 py-1 rounded-md bg-primary/10 text-primary font-bold text-sm">{calcAverage()} / 10</div>
            </div>
            {getRatingCategories(watchedCategory).map((cat) => (
              <div key={cat.key} className="space-y-1">
                <div className="flex justify-between items-start text-xs gap-2">
                  <div><p className="font-medium">{cat.label}</p><p className="text-[10px] text-muted-foreground leading-snug mt-0.5">{cat.desc}</p></div>
