...
const READ_CATEGORIES = ["webtoon", "manhwa", "manhua", "manga", "anime", "webnovel"] as const;
...
export default function ToRead() {
  ...
  const toReadArray = useMemo(() =>
    (Array.isArray(toReadList) ? toReadList : []).filter(
  ...
  const filtered = useMemo(() => {
    let items = toReadArray;
    if (activeCategory) items = items.filter((i) => i.category === activeCategory);
    if (activeGenre) items = items.filter((i) => i.genres?.includes(activeGenre));
    return items;
  }, [toReadArray, activeCategory, activeGenre]);
      ...
      {isLoading ? (
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-muted-foreground text-sm max-w-sm">When friends recommend something to read, add it here.</p>
          </CardContent>
        </Card>
      ) : filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          <p className="text-muted-foreground text-sm">No titles match your filters.</p>
          <button className="text-xs text-primary mt-2 hover:underline" onClick={() => { setActiveCategory(null); setActiveGenre(null); }}>Clear filters</button>
        </div>
      ) : (
          ...
          {Object.entries(grouped).map(([category, items]) => (
            <div key={category}>
              <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
                <span className={cn("w-2 h-2 rounded-full inline-block", CATEGORY_DOT[category] ?? "bg-muted")} />
                {category}
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {items.map((item) => (
                  <div key={item.id} data-testid={`to-read-card-${item.id}`}
